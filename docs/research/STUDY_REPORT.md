...
The exact tables are in `results/research/publication_v1/`; the overview and intervals are generated
by `tools/build_research_assets.py`.
...
demographic metadata permits subgroup fairness analysis. The study evaluates corpus labels, not
...
candidate gates are retained. Prediction ledgers, selection traces, package versions, hardware,
seeds, code commits, and output hashes permit exact metric reconstruction without releasing raw
dialogue text.
